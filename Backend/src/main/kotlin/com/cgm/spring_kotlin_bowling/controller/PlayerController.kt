...
class PlayerController(private val playerService: PlayerService){
    ...
    fun getAllFrames() : ResponseEntity<Any> {
        val scoreboard = com.cgm.spring_kotlin_bowling.jsonApiModels.Scoreboard()
        scoreboard.links.self = "$baseLink/scoreboard/frames"
        val frameList = this.playerService.getScoreBoard()
            frameList.forEach { frame ->
                val dataFrame = com.cgm.spring_kotlin_bowling.jsonApiModels.DataFrame()
    ...
    fun getRoll(@RequestBody roll : com.cgm.spring_kotlin_bowling.jsonApiModels.Roll) : ResponseEntity<Any> {
    ...
    fun getFrame(@PathVariable id: String) : ResponseEntity<Any> {
        ...
        return if ((id == "last" && lastID != 0) || (id.toInt() in 1 until lastID)) {
            val frameJson = com.cgm.spring_kotlin_bowling.jsonApiModels.FrameJson()
...
//        }
    ...
    fun createLastFrame(@RequestBody frame: com.cgm.spring_kotlin_bowling.jsonApiModels.NoIdFrameJson) : ResponseEntity<Any> {
        return APInotImplementedResponse()
    }
